...
def sim_onoff_nutlin(oscillationer, A_ext, OOmega):
    ...
    for i in range(no_half_periods + 1):
        if i == 0:
            ...
            sys = solve_ivp(system_nutlin_free, (0, half_period), state, method='LSODA', max_step=2, dense_output=True)
        elif (i != 0 and i % 2 == 0):
            state = history["p"][-1], history["m"][-1], history["Mf"][-1], history["Mb"][-1], A_ext
            sys = solve_ivp(system_nutlin_free, (0, half_period), state, method='LSODA', max_step=2, dense_output=True)
        else:
            state = history["p"][-1], history["m"][-1], history["Mf"][-1], history["Mb"][-1], 0
            sys = solve_ivp(system_nutlin_forced_zero, (0, half_period), state, method='LSODA', max_step=2, dense_output=True)
...
def worker(args):
    ...
    for i in tqdm(range(len(omega_list))):
        for j in range(len(A_ext_list)):
            ...
            window = 2000
            peaks_internal = len(find_peaks(p_sim[window:], height=np.mean(p_sim[window:]), prominence = 10)[0])
            peaks_external = len(find_peaks(n_sim[window:], height=np.mean(n_sim[window:]))[0])
...
if __name__ == "__main__":
    ...
    _, coupling_strengths = np.split(np.linspace(0.001, 5, antal_A_ext), 2)    
    ...
    save_data(arnold_dict_parallel, f'arnold/p53_no_chaos_002_{antal_omegaer}by{antal_A_ext}.pkl.gz')